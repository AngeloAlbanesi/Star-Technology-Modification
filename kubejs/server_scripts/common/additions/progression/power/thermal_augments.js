ServerEvents.recipes(event => {
    ...
    //RFC kits
    event.shaped(Item.of('kubejs:ulv_rfc_kit', '{AugmentData:{Type: RF, RFMax:4000f, RFXfer:4000f}}'), [
        ' G ',
        'SCS',
        ' G '
    ], {
    ...
    //RFS
    event.shaped(Item.of('kubejs:ulv_rfs_kit', '{AugmentData:{Type: RF, RFMax:6000f, RFXfer:2000f}}'), [
        ' S ',
        'GCG',
        ' G '
    ], {
    ...
    //RFT
    event.shaped(Item.of('kubejs:ulv_rft_kit', '{AugmentData:{Type: RF, RFMax:2000f, RFXfer:6000f}}'), [
        ' S ',
        'SCS',
        ' G '
    ], {
    ...
    [
        ...
        event.recipes.gtceu.alloy_smelter(id(`${foo.tier}_rfc_kit`))
            ...
            .itemOutputs(Item.of(`kubejs:${foo.tier}_rfc_kit`, `{AugmentData:{Type: RF, RFMax:${foo.Avg * 1000}f, RFXfer:${foo.Avg * 1000}f}}`))
            .duration(600)
        ...
        event.recipes.gtceu.alloy_smelter(id(`${foo.tier}_rfs_kit`))
            ...
            .itemOutputs(Item.of(`kubejs:${foo.tier}_rfs_kit`, `{AugmentData:{Type: RF, RFMax:${foo.Max * 1000}f, RFXfer:${foo.Min * 1000}f}}`))
            .duration(600)
            .EUt(global.va[foo.energy]);

        event.recipes.gtceu.alloy_smelter(id(`${foo.tier}_rft_kit`))
            .itemInputs(`kubejs:${foo.last_tier}_rft_kit`, `2x gtceu:${foo.metal}_gear`)
            .itemOutputs(Item.of(`kubejs:${foo.tier}_rft_kit`, `{AugmentData:{Type: RF, RFMax:${foo.Min * 1000}f, RFXfer:${foo.Max * 1000}f}}`))
            .duration(600)
            .EUt(global.va[foo.energy]);
...
const thermalKitData = {
    'ulv_rfc_kit': { max: 4000.0, xfer: 4000.0 },
    'lv_rfc_kit':  { max: 8000.0, xfer: 8000.0 },
    'mv_rfc_kit':  { max: 12000.0, xfer: 12000.0 },
    'hv_rfc_kit':  { max: 16000.0, xfer: 16000.0 },
    'ev_rfc_kit':  { max: 20000.0, xfer: 20000.0 },
    'iv_rfc_kit':  { max: 24000.0, xfer: 24000.0 },

    'ulv_rft_kit': { max: 2000.0, xfer: 6000.0 },
    'lv_rft_kit':  { max: 4000.0, xfer: 10000.0 },
    'mv_rft_kit':  { max: 6000.0, xfer: 14000.0 },
    'hv_rft_kit':  { max: 8000.0, xfer: 18000.0 },
    'ev_rft_kit':  { max: 10000.0, xfer: 22000.0 },
    'iv_rft_kit':  { max: 12000.0, xfer: 26000.0 },

    'ulv_rfs_kit': { max: 6000.0, xfer: 2000.0 },
    'lv_rfs_kit':  { max: 10000.0, xfer: 4000.0 },
    'mv_rfs_kit':  { max: 14000.0, xfer: 6000.0 },
    'hv_rfs_kit':  { max: 18000.0, xfer: 8000.0 },
    'ev_rfs_kit':  { max: 22000.0, xfer: 10000.0 },
    'iv_rfs_kit':  { max: 26000.0, xfer: 12000.0 }
};

ItemEvents.rightClicked(event => {
    let item = event.item;
    if (item && item.id.startsWith('kubejs:') && (item.id.endsWith('_rfc_kit') || item.id.endsWith('_rft_kit') || item.id.endsWith('_rfs_kit'))) {
        let name = item.id.replace('kubejs:', '');
        if (thermalKitData[name]) {
            let target = thermalKitData[name];
            let upgraded = Item.of(item.id, `{AugmentData:{Type:"RF",RFMax:${target.max}f,RFXfer:${target.xfer}f}}`);
            item.setTag(upgraded.nbt);
            event.player.tell(Text.green(`Potenziato ${item.id} a x1000! (RFMax: ${target.max}, RFXfer: ${target.xfer})`));
        }
    }
});
...
PlayerEvents.tick(event => {
    ...
    if (player.age % 20 === 0) {
        for (let i = 0; i < player.inventory.containerSize; i++) {
            ...
            if (item && !item.empty && item.id.startsWith('kubejs:')) {
                let name = item.id.replace('kubejs:', '');
                if (thermalKitData[name]) {
                    let target = thermalKitData[name];
                    let nbt = item.nbt;
                    let currentXfer = (nbt && nbt.AugmentData) ? nbt.AugmentData.getFloat('RFXfer') : 0;
                    if (currentXfer < 1000) {
                        let upgraded = Item.of(item.id, `{AugmentData:{Type:"RF",RFMax:${target.max}f,RFXfer:${target.xfer}f}}`);
                        item.setTag(upgraded.nbt);
                    }
